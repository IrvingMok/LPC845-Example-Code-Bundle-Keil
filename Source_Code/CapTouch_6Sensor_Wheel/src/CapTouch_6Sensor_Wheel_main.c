...
//
int main(void) {
  uint32_t ctrl_reg_val, poll_tcnt_reg_val, x;
  	
  // Configure the debug uart (see Serial.c).
  setup_debug_uart();
  printf("\033[2J"); // Clears the screen
  printf("CapTouch_6Sensor_Wheel example\n\r");
  ...
  // Setup SCT for PWM and start it
  Setup_SCT_PWM();
...
//ctrl_reg_val |= (X8_ACTV)<<XPINSEL;
  ...
  printf(" S0    S1    S2    S3    S4    S5   Timeouts    Overruns    TCNT    Slice\n\r");
  printf("----  ----  ----  ----  ----  ----  --------    --------    ----    -----\n\r");
