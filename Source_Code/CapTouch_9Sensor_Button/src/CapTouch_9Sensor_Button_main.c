...
int main(void) {
  ...
  printf("CapTouch_9Sensor_Button example\n\r");
  ...
  while(1) {
    ...
    if (touching) {
      ...
      LPC_GPIO_PORT->SET[0] = ~(1<<(largest + 15)); // Others off
      printf("%4d  ", largest);
    }
    else {
    ...
    //printf("%4d  %4d  %4d  %4d\r", last_touch_cnt[NUM_SENSORS-1]&0xFFF, to_cnt, ovr_cnt, touch_threshold);
