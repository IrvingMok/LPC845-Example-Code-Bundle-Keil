...
extern bool in_lp_mode;
extern uint32_t notouch_baseline[NUM_SENSORS];
...
void CAPT_IRQHandler(void) {
  ...
  if (decim_counter[current_x] == 0) {
      ...
      // Better yet, find the sensor with the smallest drop in count from its own notouch_baseline
      largest = 0;
      for (n=0; n!=NUM_SENSORS-1; n++) {
        //if (find_larger(last_touch_cnt[n+1], last_touch_cnt[largest]))
        if (find_smaller((notouch_baseline[n+1]-last_touch_cnt[n+1]), (notouch_baseline[largest]-last_touch_cnt[largest])))
          largest = n+1;
      }
