...
* main() routine
  ...
  3. SPI1 and its clocking are configured for 1 mpbs, slave mode, 8 data bits.
  
  4. SPI1 RXREADY interrupt is enabled, SPI1 interrupt is enabled as wakeup source, and code execution enters the main while(1) loop.
  
  5. When code execution receives handshake from the ISR, the requested low power mode is entered.
  
  6. The next SPI1 data received will wake up the lpc8xx, and the process repeats.
