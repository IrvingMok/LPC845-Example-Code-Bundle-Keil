...
uint32_t notouch_baseline[NUM_SENSORS];
uint32_t mean_notouch_baseline;
uint32_t ntch_noise_margin, touch_threshold;
...
uint32_t old_data[NUM_SENSORS][NUM_SAMPLES] = {0};
...
void Compute_Notouch_Baseline(void) {
...
#if USE_AVERAGED == 1
  ntch_noise_margin = ((ntch_high - ntch_low)*NM_MULTIPLIER)/NM_FRACTION;
#else
  ntch_noise_margin = ((saved_high_cnt - saved_low_cnt)*NM_MULTIPLIER)/NM_FRACTION;
#endif
  ...
  // Preload filter[k][n] with a reasonable, non-zero number 
  for (k=0; k!=NUM_SENSORS; k++) {
    for (n=0; n!=NUM_SAMPLES; n++) {
      filter[k][n] = mean_notouch_baseline;
    }
  }
